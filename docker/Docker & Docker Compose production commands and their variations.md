# 🐳 Docker & Docker Compose Commands – Production Reference
...
### 🖼️ `docker images` / `docker inspect`

List images or inspect container metadata.
docker images
...
- Use `--env-file` to inject environment variables safely.
    
- Avoid using `latest` tag in production. Use semantic version tags (`:1.0.2`).
    
- Always use volumes for persistent storage (e.g., databases).
    
- Use `restart: unless-stopped` in `docker-compose.yml`.
    
- Use `.dockerignore` to exclude unnecessary files.
    
- Configure healthchecks in Dockerfiles or Compose.
    
- Use centralized logging (e.g., ELK, Loki).
    
- Monitor with `docker stats`, Prometheus, or cAdvisor.
